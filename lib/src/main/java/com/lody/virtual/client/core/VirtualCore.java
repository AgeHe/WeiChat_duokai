...
public final class VirtualCore {
    ...
    public boolean createShortcut(int userId, String packageName, Intent splash, OnEmitShortcutListener listener) {
        ...
        shortcutIntent.setClass(context, Constants.SHORTCUT_PROXY_ACTIVITY);
        ...
        VLog.d("jyh createShortcut  ", "Userid====" + userId + "======packageName======" + packageName+"=====VUserHandle.myUserId()==="+VUserHandle.myUserId());
        VLog.d("jyh createShortcut name ", name);
        Intent addIntent = new Intent();
        addIntent.setAction("com.android.launcher.action.INSTALL_SHORTCUT");
        addIntent.putExtra(Intent.EXTRA_SHORTCUT_INTENT, shortcutIntent);
        addIntent.putExtra(Intent.EXTRA_SHORTCUT_NAME, name);
        addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON, icon);
        addIntent.putExtra("duplicate", false);//设置是否重复创建
...
//        intent.addCategory("android.intent.category.LAUNCHER");
        ...
        shortcutIntent.putExtra("_VA_|_uri_", targetIntent.toUri(0));
        shortcutIntent.putExtra("_VA_|_user_id_", userId);
        VLog.d("jyh remove  ", "Userid====" + userId + "======packageName======" + packageName);
        VLog.d("jyh remove name ", name);
        Intent addIntent = new Intent();
